...
void ordenamientoEmpleado(eEmpleado[], int);
...
void mostrarEmpleados(eEmpleado vec[], int tam)
{
    ...
    printf("  Legajo  Nombre   Sexo   Sueldo   Fecha de Ingreso  Id \n\n");
    for(int i=0; i< tam; i++)
    {
        if(vec[i].isEmpty == 0)
        {
...
{
    printf("  %4d     %s     %c  %10.2f    %02d/%02d/%4d       %d \n", emp.legajo, emp.nombre, emp.sexo, emp.sueldo, emp.fechaIngreso.dia, emp.fechaIngreso.mes, emp.fechaIngreso.anio, emp.id );
}
...
void listarSectores (eEmpleado vec[], int TAM)
{
    for(int i=o; i<5; i++)
    {
        for(int j=0; j<TAM; j++)
        {
            if(sector[i].id==empleado[j].idSector)
            {
                printf("En el sector %d hay %d empleados!");
                scanf("%d",sector[i].empleado);
            }
        }
    }
}
